...
const OUT_DIR = "ui/public/uploads/morgan/03_endpoint";
const LEDGER_PATH = path.join(OUT_DIR, "cloudflare-condition-videos-ledger.json");
const CONDITIONS = ["login", "saved-access", "quick-tunnel", "local"];
...
function parseArgs(argv) {
  const options = {
    ...
    seed: 1246,
    videoPrompt:
      "Morgan, a calm technical guide, explains the selected Cloudflare endpoint path with natural head motion, friendly eye contact, and subtle clipboard gestures; preserve outfit, leather gloves, identity, and framing",
    stylePrompt:
      "clean product setup narration, low-cognition walkthrough, soft studio lighting, neutral background, professional SaaS onboarding avatar",
    rateLimitMaxWaitSeconds: 600,
  };
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--conditions" && next) {
      ...
      console.log(`Generate Morgan Cloudflare endpoint condition MP4s with P-Video Avatar.\n\nOptions:\n  --conditions <list>  Comma list: ${CONDITIONS.join(",")}\n  --force              Regenerate existing outputs\n  --submit-only        Upload/submit without waiting\n  --wait-only          Wait/download existing ledger jobs\n`);
      process.exit(0);
    } else {
  ...
  for (const condition of options.conditions) {
    if (!CONDITIONS.includes(condition)) throw new Error(`Unknown Cloudflare condition ${condition}`);
  }
  return options;
}
...
async function uploadAudio(client, condition, entry) {
  const audioPath = path.join(OUT_DIR, `cloudflare-${condition}.mp3`);
  if (!(await pathExists(audioPath))) throw new Error(`cloudflare-${condition} missing ${audioPath}`);
  console.log(`[cloudflare:${condition}] uploading audio ${audioPath}`);
  const response = await client.uploads.uploadFile({
    file: audioPath,
    fileName: `morgan-cloudflare-${condition}.mp3`,
...
async function submitJob(client, condition, entry, options) {
  if (!entry.audioAssetId) throw new Error(`[cloudflare:${condition}] missing audioAssetId`);
  const body = {
  ...
  console.log(`[cloudflare:${condition}] submitting ${MODEL_ALIAS} job with audio ${entry.audioAssetId}`);
...
async function waitForJob(client, condition, entry) {
  if (!entry.videoJobId) throw new Error(`[cloudflare:${condition}] missing videoJobId`);
  console.log(`[cloudflare:${condition}] waiting for job ${entry.videoJobId}`);
  ...
  if (TERMINAL_FAILURES.has(job.status)) {
    throw new Error(`[cloudflare:${condition}] Scenario job ${job.jobId} ended with ${job.status}: ${job.metadata?.error ?? "<no error>"}`);
  }
  if (!entry.videoAssetId) throw new Error(`[cloudflare:${condition}] Scenario job ${job.jobId} succeeded without assetIds[0]`);
  console.log(`[cloudflare:${condition}] completed ${job.jobId} -> ${entry.videoAssetId}`);
}

async function downloadVideo(client, condition, entry) {
  const outPath = path.join(OUT_DIR, `cloudflare-${condition}.mp4`);
  if (!entry.videoAssetId) throw new Error(`[cloudflare:${condition}] missing videoAssetId`);
  ...
  console.log(`[cloudflare:${condition}] downloaded ${entry.videoAssetId} -> ${outPath}`);
}
...
async function main() {
  ...
  for (const condition of options.conditions) {
    const outPath = path.join(OUT_DIR, `cloudflare-${condition}.mp4`);
    const entry = { condition, ...(ledger.conditions[condition] ?? {}) };
    ledger.conditions[condition] = entry;
    if (!options.force && options.submit && await pathExists(outPath)) {
      console.log(`[cloudflare:${condition}] local MP4 exists; use --force to regenerate.`);
      entry.videoPath = outPath;
    ...
    if (options.submit) {
      await uploadAudio(client, condition, entry);
      await saveLedger(ledger);
      await submitJob(client, condition, entry, options);
      await saveLedger(ledger);
    }
    if (options.wait) {
      await waitForJob(client, condition, entry);
      await saveLedger(ledger);
    }
    if (options.download) {
      await downloadVideo(client, condition, entry);
      await saveLedger(ledger);
    }
  }
