...
- **Credential/setup wizard:** `LocalStackBootstrap` now starts with a large animated 5D hero, then moves through Secrets, endpoint, Source, harness, Dynamic workflows, providers/models/routing/auth, tools, and agent-token setup before the final Start step.
- **Dynamic workflows model:** The setup UI names the workflow layer directly. Claude Code, Copilot, Codex, Gemini, OpenCode, and similar integrations are selectable agent surfaces under that workflow, not the workflow name. GitHub/GitLab source helpers stay in Source setup.
